...
3. **Store the credentials** in the **org base** environment. Webhook-triggered runs (the
   `github-dispatcher` -> `code-factory` chain) always run in the base; a named environment only
   applies to manual `run --environment <name>` or scheduled runs, so omit `--environment` here. See
   [`.env.example`](./.env.example) for the local `boardwalk dev` equivalents.
   - `GITHUB_APP_ID` is a non-secret variable: `boardwalk variables set GITHUB_APP_ID <id>`
   - `GITHUB_APP_PRIVATE_KEY` is a secret: `boardwalk secrets set GITHUB_APP_PRIVATE_KEY`
